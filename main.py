...
def get_binance_data(endpoint, exchange_info_url):
    try:
        exchange_info = requests.get(exchange_info_url, timeout=10).json()
        active_symbols = {s['symbol'] for s in exchange_info.get('symbols', []) if s['status'] == 'TRADING'}

        data = requests.get(endpoint, timeout=10)
...
def send_to_telegram():
    try:
        spot = get_binance_data("https://api.binance.com/api/v3/ticker/24hr", "https://api.binance.com/api/v3/exchangeInfo")
        fut = get_binance_data("https://fapi.binance.com/fapi/v1/ticker/24hr", "https://fapi.binance.com/fapi/v1/exchangeInfo")
    except Exception as e:
